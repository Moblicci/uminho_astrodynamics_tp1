...
def get_gravitational_force_vector(μ, body_position_vector):
    scalar_distance = np.linalg.norm(body_position_vector)
    if scalar_distance == 0:
        return np.array([0.0, 0.0])
    return -μ * body_position_vector / scalar_distance**3 # F = -μ * r / |r|^3
...
def orbital_state_derivates(time, ship_orbital_space_vector):
    ...
    gravitational_force_vector_earth = get_gravitational_force_vector(μ_earth, r_earth_vector)
    ...
    if distance_to_moon < 30000: 
        gravitational_force_vector_moon = get_gravitational_force_vector(μ_moon, r_moon_vector)
    else:
        gravitational_force_vector_moon = np.array([0.0, 0.0])

    gravitational_acceleration_x, gravitational_acceleration_y = (gravitational_force_vector_earth + gravitational_force_vector_moon)
...
apogee_gravitational_force = get_gravitational_force_vector(μ_earth, apogee_position)
